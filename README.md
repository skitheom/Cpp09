...
これ以上はpair化できないため、pairingは終了

#### Insertion

これ以上のpairingができなくなった段階で、Leader によって構成された主鎖（main chain）が完成している。Follower（各ペアの小さい方の要素）および最後に残った余剰要素は、Jacobsthal 数列に基づいた順序で、**二分探索によって**主鎖に挿入される
...
{b_1, a_1, a_2} はsort済み
b_2にあたる`17`と、余剰要素の`19`のみ挿入対象になる
